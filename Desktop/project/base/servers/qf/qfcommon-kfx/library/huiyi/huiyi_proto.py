...
class HuiyiProto(object):
    ...
    def huiyi2qf(self, req_data, resp_str, mac_checker=None):
        ...
        def _update_result_data(result_part):
        ...
        log.debug('huiyi2qf result=%s', result_data)
        return result_data
    ...
    def _get_value_from_huiyi(self, index, resp8583):
        ...
        try:
            ret = self._get_value_from_huiyi_imp(index, resp8583)
            if isinstance(ret, tuple):
                ...
                raise py8583_err.Py8583ProgramError('invalid return value from _get_value_from_huiyi_imp')
        ...
        if index == 55:
            ...
            if iccdata:
                ...
                # 冲正需要剔除一些TLV
                    ...
                    new_iccdata = huiyi_field55.build(iccdata_data)
    ...
    def _get_value_from_huiyi_imp(self, index, resp8583):
        ...
        # acq_inst_resvd,  受理方保留
        if index == 122:
            # value 形如： '\xd2\xf8\xc1\xaa\xbb\xfa\xb9\xb9\xba\xc5:48290000\n\xd2\xf8\xc1\xaa\xc9\xcc\xbb\xa7\xba\xc5:         \n829588255410540'
            # 数据是GBK编码的

            u = value.decode('gbk')
            org_part, mch_part = u.split('\n', 1)
            _, orgid = org_part.split(':')
            _, mchid = mch_part.split(':')
            orgid = orgid.strip()
            mchid = mchid.strip()

            return {
                'unionpay_orgid': orgid,
                'unionpay_mchid': mchid,
            }
