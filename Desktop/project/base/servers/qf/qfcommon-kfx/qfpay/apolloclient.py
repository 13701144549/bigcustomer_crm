...
class Apollo(object):
    ...
    def pre_register(self, username, password, email):
        """预注册接口.
        params:
            username: str, 用户名.
            password: str, 密码.
        ...
        userid = None
        try:
            userid = thrift_callex(self.server, ApolloServer, 'preRegister', username, password, email)
        except ApolloException as e:
            log.debug('preregister error: %s', e)
            if e.respcd == ApolloRet.APOLLO_ERR_USER_EXIST:
                return None, '商户已经注册'
        else:
            log.debug('preregister userid: %s', userid)
            return userid, None
    ...
    def change_pwd(self, username, old_password, new_password):
        ...
        '''
        try:
            user = self.auth(username, old_password, expire=86400 * 3)
            if not user:
                raise ValueError('old password is not true')
            ret = ThriftClient(self.server, ApolloServer).call('changePwd', user.get('uid'), new_password)
            if ret == 0:
                return True
            else:
    ...
    def user_by_shopname(self, shopname, groupid):
        """ 根据店铺名查询用户基本信息.
        params:
            shopname: str, 店铺名
            groupid: int, groupid, 即所属一级渠道的 id.
        """
        users = None
        try:
            ret = ThriftClient(self.server, ApolloServer).call('findUserByShopname', shopname, groupid)
            users = [user.__dict__ for user in ret]
        except:
            log.debug(traceback.format_exc())
        return users

    def user_by_name(self, name, groupid):
        """ 根据名称查询用户基本信息.
        params:
            name: str, 店铺名
            groupid: int, groupid, 即所属一级渠道的 id.
        ...
        users = None
        try:
            ret = ThriftClient(self.server, ApolloServer).call('findUserBriefsByName', name, groupid)
            users = [user.__dict__ for user in ret]
        except:
            log.debug(traceback.format_exc())
        return users

    def user_by_condition(self, spec):
        """ 根据条件查询用户基本信息.
        params:
            spec: 包含查询条件的 json 字符串.
        """
        users = None
        try:
            ret = ThriftClient(self.server, ApolloServer).call('findUserBriefs', spec)
            users = [user.__dict__ for user in ret]
        except:
            log.debug(traceback.format_exc())
        return users
    ...
    def set_user_state(self, uid, state):
        ...
        """
        try:
            ret = ThriftClient(self.server, ApolloServer).call('setUserState', int(uid), int(state))
            return ret == 0
        except:
            log.debug(traceback.format_exc())
            return False
    ...
    def update_user(self, uid, values):
        ...
        try:
            ...
            ret = ThriftClient(self.server, ApolloServer).call('updateUser', int(uid), user)
            return ret == 0
        except:
            log.debug(traceback.format_exc())
            return False

    def get_user_ext(self, uid):
        """ 获取用户拓展信息.
        """
        try:
            ret = ThriftClient(self.server, ApolloServer).call('getUserExt', int(uid))
            return ret.__dict__
        except:
            log.debug(traceback.format_exc())
            return None
    ...
    def __call__(self, method, *args, **kw):
        ...
        if callable(func):
            ...
            return thrift_callex(self.server, ApolloServer, method, *args, **kw)
