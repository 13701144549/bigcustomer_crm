...
class LocationKey(object):
    ...
    MEMBER_KEY = 'location-member'  # 欢迎消息中的会员  1
    ADV_KEY    = 'location-adv'     # 欢迎消息中的广告  1
    SALE_KEY   = 'location-sale'    # 欢迎消息会员中的特卖 2
    COUPON_KEY = 'location-coupon'  # 欢迎消息会员中的红包 2

    _KEYS = (MASTER_KEY, MEMBER_KEY, ADV_KEY, SALE_KEY, COUPON_KEY)
...
class MPClient(object):
    ...
    def reply_save_location(self, key, title='', tail='', link='', weight=-1, content='', status=MP_ENABLE):
        ...
        req_args.push_txt = json.dumps(data)
    ...
    def reply_query_location(self, key=None, page_curr=1, page_size=20):
        ...
        def _push_txt_str2dict(push_txt):
            '''push_txt为文本的解析'''
            data = {
                'title': '',
                'tail' : '',
            ...
            if item_cnt == 2:
                if push_txt_item[1]:
                    ...
                    a_tag = soup.a
                    if a_tag:
                        data['tail'] = a_tag.text
                        data['link'] = a_tag['href']
                        # thrift请求，返回值都是utf-8，所以这里也把编码统一为utf-8
                        if isinstance(data['tail'], unicode):
                            data['tail'] = data['tail'].encode('utf-8')
                        if isinstance(data['link'], unicode):
                            data['link'] = data['link'].encode('utf-8')
                    else:
                        data['tail']= push_txt_item[1]
            return data
        ...
        for item in resp_data.data:
            ...
            if item.push_type != ReplyType.TEXT:
                continue
            try:
                push_data = json.loads(item.push_txt)
            except:
                # 非json格式，转json
                push_data = _push_txt_str2dict(item.push_txt)
            item.extitle = push_data['title']
            item.extail  = push_data['tail']
            item.exlink  = push_data['link']
        return resp_data
