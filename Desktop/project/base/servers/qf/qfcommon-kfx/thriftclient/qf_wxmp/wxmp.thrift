...
struct TemplateSend
{
    ...
    6: optional i16    type=0,      // 消息类型 0:微信正常推送 1:小程序推送
    7: optional string form_id,     // 小程序用到的form_id
    8: optional string ext,         // 额外信息
}
...
struct Tag
{
    ...
    2: required string name,   // 标签名
    3: required i64    count,  // 标签下粉丝数
}
...
service QFMP
{
    ...
    // 查询公众号配置
    list<MPConf> mp_deep_query(1: i32 userid) throws(1:MPError e);
    // 获取微信的access_token
    // 返回值: 公众号的access_token与ttl
    ...
    // 生成小程序二维码
    // 参数 scene, page, width, auto_color, line_color, is_hyaline
    // 文档:https://developers.weixin.qq.com/miniprogram/dev/api/qrcode.html?search-key=%E4%BA%8C%E7%BB%B4%E7%A0%81
    // 返回值图片二进制 
    binary get_wxop_qrcode(1: string appid, 2:string params) throws(1:MPError e);
    // 授权公众号绑定到商户
    // 返回值 0: 成功  <0与异常: 失败
    i16 mp_auth_bind(1: MPAuthBind req_args) throws(1:MPError e);
    // 绑定mp_user 
    // 暂无限制一个商户是否能绑定多个公众号
    i16 batch_mp_user_bind(1:string appid, 2:list<i64> userids) throws(1:MPError e);
    // 绑定mp_user ext
    // 指定appid, 只保留商户和改appid的关系
    i16 auth_user_bind_ex(1:string appid, 2:i64 userid) throws(1:MPError e);
    // 解绑mp_user
    // 如果appid为空, 将解绑userid所有的关系
    i16 batch_mp_user_unbind(1:string appid, 2:list<i64> userids) throws(1:MPError e);
    // 获取mp_conf
    map<i64, list<MPConf>> batch_mp_query(1:list<i64> userids) throws(1:MPError e); 
    ...
    // 获取微信用户数据
    // 返回微信返回的json后的字符串
    string get_userinfo(1: string appid, 2: string openid) throws(1:MPError e);
    ...
    // 如果需要送，则发送消息
    // 返回值 0: 发送成功 异常失败
    i16 coupon_subscribe(1: string appid, 2: string openid) throws(1:MPError e);
    ...
    // 推送客服消息
    i32 push_custom(1:string params) throws(1:MPError e);
    // 查看推送结果
    // 返回值: pushid对应的推送结果(推送总数，真正推送数，成功数，失败数)
    map<i64, PushStat> push_stat(1: string appid, 2: list<i64> pushids) throws(1:MPError e);
    // 获取appid的标签
    list<Tag> get_tags(1: string appid) throws (1:MPError e);
    // 给消费者打上对应的标签
    i32 set_tag(1: string appid, 2:list<string> openids, 3:i64 tagid) throws(1:MPError e);
    ...
    // 企业付款
    // 参数 spec: json.dumps({'openid':, 'key':, 'mchid': 'amt':, 'desc':})
    // openid, key, mchid, amt 必填  desc选填
    // 返回值 0: 发送成功
    i32 pay_transfer(1: string spec) throws(1:MPError e);
}
