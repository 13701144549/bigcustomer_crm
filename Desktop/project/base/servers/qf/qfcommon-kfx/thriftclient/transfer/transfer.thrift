...
struct PayDownload{
    1: optional string bat_id,    //批次号
    2: optional string stime,     //开始时间戳
    3: optional string etime,     //结束时间戳
    4: optional string state,     //代付状态
}

struct WithDrawArgs{
    1:required string biz_sn,
    2:required string withdraw_sn,
    3:required i32 amt,
...
service TransFerService {
    ...
    //异步获取可以出款的总数，以及总金额(按账户类型)
    oneway void get_can_transfer_byaccount(),

    //获取可提现的 出款信息
    oneway void get_withdraw_info(),
    //生成可提现的出款记录
    oneway void gen_withdraw_record(1:list<WithDrawArgs> arg,2:string rule),
    ...
    //补款添加纪录
    i32 add_supplement_record(1:required i64 userid, 2:required i64 amt, 3:required string reason, 4:required i64 account_type),
    //补款提审
    ...
    // 生成代付文件
    oneway void generate_payanother_excel(1:required PayDownload params),
}
