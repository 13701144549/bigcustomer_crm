...
// 按帐户类型统计帐户
struct AccountTypeStat
{
    1: i64       cnt,       // 总数
    2: i64       amt,       // 总金额
    3: list<i64> userids,   // 商户id列表
}
...
struct Transfer
{
    ...
    16: optional string chnl_code,          // 结算通道码
}
...
struct SettleCycle
{
    1: SettleCycleType type = SettleCycleType.MONTH,        // 结算周期类型 
    2: i32 value,    // 结算日期, 按月结，则为每月几号
}
...
{
    1: i64 userid,                  // 用户id
    2: SettleCycle settle_cycle,    // 结算周期
    3: i32 settle_threshold = -1,   // 结算门槛，单位为分
    4: list<FeeRatio> fee_ratio,    // 费率
    5: StatusType status = StatusType.ENABLE,   // 状态 0: 不启用 1: 启用
}
...
service UserAccount
{
    ...
    // 返回所有userid的帐户金额大于0的
    // 如果为空list，则查所有的
    map<i64, AccountTypeStat> account_type_stat(1:list<i64> acct_type_ids) throws(1:ServerError e);
    ...
    // 提现返回
    // 必传参数: ['userid', 'amt', 'txcurrcd', 'biz_sn', 'orig_biz_sn', 'sysdtm', ]
    ...
    // 保存帐户结算属性
    // 注: 不能通过此接口保存商户费率，保存商户费率应通过fee_ratio_save接口保存
    //     如果只不更新结算周期，则settle_cycle为None
    //     如果不更新settle_threshold，则为-1
    i16 fund_profile_save(1:FundProfile req_args) throws(1:ServerError e)
    // 查询帐户结算属性
    // 目前只支持查询启用的属性
    // XXX: 这个接口设计的不好，因为成都那边已开发，所以不方便再修改
    //      以后需要需要查询所有的属性，再加接口来处理
    //      新版可以按数据库的结构来设计结构体，然后保存时，一次保存一条
    //      查询时，返回的结构体中包含两个元素，一个是属性list, 一个是费率list
    //      而且查询时，可以多加几个条件
    list<FundProfile> fund_profile_query(1:list<i64> userids) throws(1:ServerError e)
