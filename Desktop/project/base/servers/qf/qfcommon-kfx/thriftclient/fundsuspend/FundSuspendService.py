...
class Iface:
  ...
  def add_risk_records(self, riskdata):
    """
    Parameters:
     - riskdata
    """
...
class Client(Iface):
  ...
  def add_risk_records(self, riskdata):
    ...
    Parameters:
     - riskdata
    """
    self.send_add_risk_records(riskdata)

  def send_add_risk_records(self, riskdata):
    self._oprot.writeMessageBegin('add_risk_records', TMessageType.CALL, self._seqid)
    args = add_risk_records_args()
    args.riskdata = riskdata
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
    self._oprot.trans.flush()
  def audit_suspend_records(self, batid, hand_state):
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["add_risk_records"] = Processor.process_add_risk_records
  ...
  def process_add_risk_records(self, seqid, iprot, oprot):
    args = add_risk_records_args()
    args.read(iprot)
    iprot.readMessageEnd()
    self._handler.add_risk_records(args.riskdata)
    return
...
class add_risk_records_args:
  ...
  thrift_spec = (
    None, # 0
    (1, TType.STRUCT, 'riskdata', (RiskRecord, RiskRecord.thrift_spec), None, ), # 1
  )

  def __init__(self, riskdata=None,):
  ...
  def read(self, iprot):
    ...
    while True:
      ...
      if fid == 1:
        if ftype == TType.STRUCT:
          self.riskdata = RiskRecord()
          self.riskdata.read(iprot)
        else:
          iprot.skip(ftype)
      else:
        iprot.skip(ftype)
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('add_risk_records_args')
    if self.riskdata is not None:
      oprot.writeFieldBegin('riskdata', TType.STRUCT, 1)
      self.riskdata.write(oprot)
      oprot.writeFieldEnd()
    oprot.writeFieldStop()
    oprot.writeStructEnd()

  def validate(self):
    if self.riskdata is None:
      raise TProtocol.TProtocolException(message='Required field riskdata is unset!')
    return
