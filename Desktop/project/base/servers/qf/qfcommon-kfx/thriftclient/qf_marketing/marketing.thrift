...
struct Activity
{
    1:  i64         id,
    2:  string      src,                 // 来源
    3:  string      mchnt_id,            // 可以分放红包的商户id, 如果为空，则说明可能有多个商户，从activity_mchnt表中取
    4:  i16         type,                // 活动类型 1:满减 2:消费返券/分享券 3: 分发活动 4: 兑换活动 5: 关注公众号送红包 11:积分
    5:  string      title,               // 活动标题
    ...
    21: string      create_mchnt_id,     // 创建活动的商户id, 如果为空，则表示是平台创建的，否则表示是商户创建的。默认与mchnt_id一致
    22: i64         union_id,             // 联盟活动id
    23: list<i64>   mchnt_list,         // 添加该活动的商户
}
...
struct Coupon
{
    ...
    14: string   use_mchnt_id,  // 使用红包的mchnt_id, 为空则代表多店可用, 否则单店使用
    15: i64      union_id,      // 联盟活动id
}
...
struct CouponRule
{
    1:  i64     id,                 // 红包规则id
    2:  string  src,                // 来源
    3:  string  mchnt_id,           // 可以使用红包的商户id, 如果为空，则说明可能有多个商户，从coupon_rule_mchnt表中取
    ...
    13: string  create_mchnt_id,    // 创建活动的商户id, 如果为空，则表示是平台创建的，否则表示是商户创建的。默认与mchnt_id一致
    14: i16     act_type,           // 要创建的活动的类型
    15: list<i64> use_userids,      // 可以使用红包的商户id(仅当mchnt_id为空时启用)
}
...
// 关注送红包的参数
// 送红包时，红包状态一定是绑定
struct CouponSubscribeArgs
{
    1: required string  src,           // 来源
    2: required string  customer_id,   // 消费者id
    3: optional string  mchnt_id = '', // 商户id
                                       //     如果存在，则为商户自己给消费者发放红包
                                       //               那活动必须是商户自己创建的单店红包活动
                                       //     如果不存在，则为平台创建的活动，那活动必须发放平台活动
    4: optional i64     activity_id,   // 活动id 如果存在，则送指定活动的，否则送启用的最后一个活动
}
...
// 可以使用红包的商户
struct CouponRuleMchnt
{
    1: required string       src,           // 来源，必填
    2: required list<string> mchnt_list,    // 添加该活动的商户
    3: required i64          rule_id,       // 红包使用规则id
    4: required i16          op_type,       // 报名操作的类型，1: 添加  2: 删除
    5: optional string       content,       // 备注
}
...
{
    1: string rulename,  // 规则名称
    2: double mchnt_rate,  // 从商户抽取的比例
    3: double send_coupon_rate,  // 发券方抽佣比例
    4: double operator_rate,  // 运营方抽佣比例
    5: double plate_rate,  // 平台方抽佣比例
    6: i32 userid,
}
...
{
    1: i64         id,
    2: i64         userid,              // 创建活动的商户id
    3: i64         cmn_id,              // 分佣id
    4: i16         type,                // 活动类型 1: 消费返红包 2: 链接分发红包
    5: string      title,               // 活动标题
    6: string      rule,                // 活动规则 {'obtain_rule': [['amt', '>=', 10], ['begin_time', '>=', 0], ['end_time', '<=', 86400], ['num_type', '=', 1], ['obtain_num', '<', '10']], 'share': [['amt', '>=', 10], ['num_type', '=', 1], ['share_num', '<', 10], ['begin_time', '>=', 0], ['end_time', '<=', 86400]]}
    7: i16         status,              // 活动状态
    8: string      start_time,          // 有效期的开始时间
    9: string      expire_time,         // 有效期的结束时间
    10: list<i64>  joined_userids,      // 参与活动的商户
}

// 核销红包活动
struct ActivityVerify {
    1:  optional i64    id;
    2:  optional i64    userid;
    3:  optional string name;
    4:  optional string src;
    5:  optional i32    type; // 优惠类型 1:折扣 2:满减
    6:  optional string info;
    7:  optional string descr;
    8:  optional i32    status;
    9:  optional string img;
    10: optional string start_time;
    11: optional string expire_time;
    12: optional string create_time;
    13: optional string update_time;
    14: optional list<i64> mchnt_id_list;
}
...
service QFMarketing
{
    ...
    // 修改红包多商户使用活动
    // 返回值: 0: 成功
    i32 activity_multimchnt_change(1:Activity activity) throws(1:ServerError e);
    ...
    // 分享活动(消费活动与满减活动)
    // 返回值: 活动的分享信息
    //         如果是消费分享，则返回share(分享信息)，page(分享页面自定义信息)
    //         如果消费返与满减，则返回coupons
    //         如果消费奖积分，则返回奖励的积分
    //         以上三种方式，可以组合，比如消费分享+消费返(app已使用)
    ...
    // 查询可分享的活动, 不验证活动规则
    // 返回值: 可分享的活动
    Activity activity_shareable_no_rule(1:ActivityShareArgs req_args) throws(1:ServerError e);

    // 查询关注可参加的活动
    // 返回值: 正在启用的关注活动的id列表
    list<i64> activity_subscribe(1:string mchnt_id);
    ...
    // 红包可以使用的商户
    // 返回值: 0: 成功
    i32 coupon_rule_mchnt(1:CouponRuleMchnt apply) throws(1:ServerError e);

    // 红包在多商户间共享使用
    // 返回值: 0: 成功
    i32 coupon_rule_multimchnt(1:CouponRuleMchnt req_args) throws(1:ServerError e);
    ...
    // 通过coupon_code查询红包
    // 返回值: 红包信息
    Coupon coupon_query_by_code(1:string code) throws(1:ServerError e);
    ...
    // 关注送红包
    // 返回值: 红包信息
    Coupon coupon_subscribe(1:CouponSubscribeArgs req_args) throws(1:ServerError e);

    // 批量关注送红包
    // 返回值: 红包信息列表
    list<Coupon> batch_coupon_subscribe(1:list<i64> actv_ids, 2:string customer_id) throws(1:ServerError e);
    ...
    // 创建联盟活动
    // 返回值: 成功活动的id
    i64 union_actv_create(1:ActivityUnion actv_union) throws(1:ServerError e);
     
    // 修改活动
    // 返回值: 0: 成功
    i32 union_actv_change(1:ActivityUnion actv_union) throws(1:ServerError e);

    // 联盟活动tip
    // 返回值: ActivityUnion 
    // customer_id, userid
    ActivityUnion union_actv_tip(1:string params) throws(1:ServerError e);
    
    // 获取联盟红包
    // 基础必须参数: customer_id, userid, txamt, out_sn, mode
    // mode为consume: 即为消费返红包
    // mode为link: 为链接返红包, link_id必传
    ActivityUnion union_actv_obtain(1:string params) throws(1:ServerError e);

    // 联盟活动添加分发链接
    // activity_id, tags
    i32 add_link(1:i64 activity_id, 2:list<string> tags) throws(1:ServerError e);
    ...
    // 创建抽佣规则
    // 返回值: 成功的规则id
    i64 commission_rule_create(1:CommissionRule commissionRule) throws(1:ServerError e);
    ...
    // 创建核销活动
    // 返回值: 成功,活动的id
    i64 verify_actv_create(1:ActivityVerify actv) throws(1:ServerError e);

    // 核销红包
    // params: json.dumps(userid, verify_id, src)
    // userid:商户id, verify_id:兑换码 src:核销来源
    i32 verify_actv_verify(1:string params) throws(1:ServerError e);

    // 分发红包
    // 返回链兑换码列表
    // params: json.dumps(actv_id, num, src)
    // actv_id:核销活动id, num:数量 src:来源
    list<i64> get_verify_code(1:string params) throws(1:ServerError e);
}
